...
public class DrawGraph extends JComponent {
	...
	public DrawGraph(QuadTree<Double> qt, int w, int h) {
		...
		nodes = queryQT(new Interval<Double>(upperLeftX, upperLeftX+utmWidth), new Interval<Double>(upperLeftY-utmHeight, upperLeftY));
		drawGraph(nodes, w, h);
	...
	public void drawGraph(ArrayList<MyNode> n, double w, double h){
		nodes = n;
		width = w;
		height = h;
		calculateFactor();
	...
	private ArrayList<MyNode> queryQT(Interval<Double> xInterval, Interval<Double> yInterval) {
		MyInterval2D<Double> rect = new MyInterval2D<Double>(xInterval, yInterval);
		return qt.query2D(rect);
	}
	...
	private void zoom() {
		...
		nodes = queryQT(new Interval<Double>(upperLeftX, upperLeftX+utmWidth), new Interval<Double>(upperLeftY, upperLeftY-utmHeight));
		drawGraph(nodes, width, height);
		System.gc();
	...
	private void resetMap() {
		...
		calculateFactor();
		nodes = queryQT(new Interval<Double>(upperLeftX, upperLeftX+utmWidth), new Interval<Double>(upperLeftY-utmHeight, upperLeftY));
		drawGraph(nodes, getWidth(), getHeight());
		repaint();
	}
	...
	public void paintComponent(Graphics2D g) {
		...
		drawGraph(nodes, (int)CreateGUI.returnInstance().getWidth(), (int)CreateGUI.returnInstance().getHeight());
		...
		try {
			for(MyNode n : nodes) {
				for(MyEdge e : n.getEdges()) {
					...
					if(!(roadType == 1 || roadType == 2 || roadType == 3 || (roadType == 4 && zoomLvl >= 300) || (roadType == 5 && zoomLvl >= 900) || (roadType == 8 && zoomLvl >= 3500) || (roadType != 8 && zoomLvl >= 2000))) { // filters different road types depending on the zoom level
						break;
					}
					if(roadType == 1) {
	...
	private class MyMouseWheelListener implements MouseWheelListener{
		...
		public void mouseWheelMoved(MouseWheelEvent e) {
			...
			double widthHeightRatio = 484790.0/379423.0; //The ratio between width and height

			if(notches < 0) { //Zoom in
				if(utmWidth < 1500 || utmHeight < 1500) { //You can't zoom more than this (1500m in either width or height)
					return;
				}
				x1 = e.getX()-pixelConstant*widthHeightRatio;
				y1 = e.getY()+pixelConstant;
				x2 = e.getX()+pixelConstant*widthHeightRatio;
				...
				utmWidth += 2*1.2777*pixelConstant*(3000/factor);
